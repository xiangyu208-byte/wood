...
def class_aware_nms(detections: Sequence[Detection], iou_threshold: float = NMS_IOU_THRESHOLD) -> list[Detection]:
    ...
    for class_id in sorted({item.class_id for item in detections}):
        ...
        while pending:
            ...
            pending = [item for item in pending if intersection_over_union(selected, item) < iou_threshold]
...
def draw_detections(
    ...
    palette = ((39, 103, 73), (31, 81, 59), (183, 121, 31), (72, 103, 122), (177, 58, 58), (92, 107, 98))
    for item in detections:
        ...
        label = f"{class_names.get(item.class_id, str(item.class_id))} {item.confidence:.2f}"
